...
run_upgrade() {
  ...
  # remove the redundant tag first — and that is the very tag the backend
  # container was created from (the retag above). `docker ps` then can't resolve
  # the container's image reference and falls back to a bare image id, so the
  # retag's whole purpose is lost on every OTA upgrade. Re-apply it: docker
  # resolves the reference at display time, so the name comes back without a
  # restart or recreate. Best-effort — never fail an otherwise-good upgrade.
  if [ -n "$target" ]; then
    docker tag "$REGISTRY/borzoi-backend:latest" \
      "$REGISTRY/borzoi-backend:$target" 2>/dev/null || true
  fi
